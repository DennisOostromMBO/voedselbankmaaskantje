            ...
            <form method="POST" action="{{ route('customers.update', $customer->id) }}">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="postcode" value="{{ old('postcode', $customer->postcode ?? '') }}" required
                            class="w-full border border-blue-200 rounded px-2 py-1 text-gray-900 focus:ring focus:ring-blue-200">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="wish" class="w-full border border-blue-200 rounded px-2 py-1 text-gray-900 focus:ring focus:ring-blue-200">
                            <option value="geen" {{ old('wish', $customer->wish ?? '') == 'geen' ? 'selected' : '' }}>Geen</option>
                            <option value="Vegetarisch" {{ old('wish', $customer->wish ?? '') == 'Vegetarisch' ? 'selected' : '' }}>Vegetarisch</option>
                            <option value="Veganistisch" {{ old('wish', $customer->wish ?? '') == 'Veganistisch' ? 'selected' : '' }}>Veganistisch</option>
                            <option value="Geen varkensvlees" {{ old('wish', $customer->wish ?? '') == 'Geen varkensvlees' ? 'selected' : '' }}>Geen varkensvlees</option>
                        </select>
                    </div>
                </div>
